...
from .enums import (
    QueryType, DataSource, ContextSufficiency,
    ActionType, TaskStatus, ReflectionType
)
...
class ContextCollection:
    ...
    def prepare_context(self) -> Dict[str, Any]:
        ...
        if self.current_document:
            context["current_document"] = json.dumps(self.current_document.to_dict(), ensure_ascii=False)
        if self.selected_content:
            context["selected_content"] = self.selected_content or ""
        if self.items:
            context["collected_contexts"] = json.dumps([item.to_dict() for item in self.items], ensure_ascii=False)
        return context  
