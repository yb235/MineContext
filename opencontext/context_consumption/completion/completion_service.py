...
class CompletionSuggestion:
    ...
    def __init__(self, text: str, completion_type: CompletionType, 
                 confidence: float, context_used: List[str] = None):
        self.text = text
...
class CompletionService:
    ...
    def get_completions(self, current_text: str, cursor_position: int, 
                       document_id: Optional[int] = None, 
                       user_context: Dict[str, Any] = None) -> List[CompletionSuggestion]:
        """
    ...
    def _analyze_document_structure(self, text: str, cursor_pos: int) -> Dict[str, Any]:
        ...
        # Detect list items
        is_in_list = bool(re.match(r'^\s*[-*+]\s', current_line)) or \
                     bool(re.match(r'^\s*\d+\.\s', current_line))
    ...
    def _get_semantic_continuations(self, context: Dict[str, Any]) -> List[CompletionSuggestion]:
        ...
        try:
            ...
            user_prompt_template = prompt_group.get('user', 'Please provide continuation suggestions for the text')
            ...
            if response and response.choices:
                ...
                for continuation in continuations[:2]:  # At most 2 semantic continuation suggestions
    ...
    def _rank_and_filter_suggestions(self, suggestions: List[CompletionSuggestion]) -> List[CompletionSuggestion]:
    ...
    def _generate_cache_key(self, context: Dict[str, Any], document_id: Optional[int]) -> str:
        ...
        key_data = {
            'context_before': context.get('context_before', '')[-100:],  # Use only the last 100 characters
            'current_line': context.get('current_line', ''),
